...
abstract class PackwizPath<T : PackwizPath<T>>(
    path: String? = null,
) {
    ...
    init {
        if (path != null) {
            // Check for NUL bytes
            if (path.contains('\u0000')) {
                throw RequestException.Validation.PathContainsNUL(path)
            }
